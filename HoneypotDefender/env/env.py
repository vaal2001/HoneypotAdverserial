...
class HoneypotDefenderEnv(gym.Env):
    ...
    def reset_with_world(self, N_actual, hosts, adjacency, seed: int | None = None):
        """
        Gebruik extern aangeleverde hosts + adjacency.

        - Zelfde post-condities als reset()
        - Rekent baseline_stats & detectability opnieuw uit.
        """
        if seed is not None:
            self.rng = np.random.default_rng(seed)

        self.N_actual = int(N_actual)
        self.hosts = list(hosts)  # shallow copy OK
        self.adjacency[:, :] = 0.0
        n = self.N_actual
        self.adjacency[:n, :n] = adjacency[:n, :n].astype(np.float32)

        # baseline-statistieken op basis van REAL hosts
        self.baseline_stats = compute_real_baseline_stats(
            self.hosts, self.adjacency, self.N_actual
        )
        ...
        # detectability initieel (baseline vóór defender-acties)
        assert self.baseline_stats is not None
        mean_h, mean_r = detectability_for_hosts(
            self.hosts,
            self.adjacency,
