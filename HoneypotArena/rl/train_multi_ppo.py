...
def main():
    ...
    # AGENTS
    # ---------------------------------------------------
    attacker = AttackerPPOAgent(
        obs_dim_att, n_actions_att, env.N_max, env.F_attacker
    ).to(device)
    defender = DefenderPPOAgent(
        obs_dim_def, n_actions_def, env.N_max, env.F_defender
    ).to(device)
    ...
    while global_step < args.total_timesteps:
        ...
        for t in range(args.num_steps):
            ...
            # naar torch
            att_obs_tensor = torch.tensor(
                obs_att_flat, dtype=torch.float32, device=device
            ).unsqueeze(0)
            att_mask_tensor = torch.tensor(
                mask_att, dtype=torch.bool, device=device
            ).unsqueeze(0)

            def_obs_tensor = torch.tensor(
                obs_def_flat, dtype=torch.float32, device=device
            ).unsqueeze(0)

            # acties kiezen
            with torch.no_grad():
                # defender actie
                a_def, logp_def, ent_def, v_def = defender.get_action_and_value(
                    def_obs_tensor
                )

                # attacker actie (met action_mask)
                a_att, logp_att, ent_att, v_att = attacker.get_action_and_value(
                    att_obs_tensor, action_mask=att_mask_tensor
                )

            act_def = int(a_def.item())
            act_att = int(a_att.item())

            # env step: defender eerst (in env-logica), maar we geven beide tegelijk
        ...
        with torch.no_grad():
            _, _, _, next_v_att = attacker.get_action_and_value(
                att_obs_tensor, action_mask=att_mask_tensor
            )
            _, _, _, next_v_def = defender.get_action_and_value(
                def_obs_tensor
            )
        ...
        for t in reversed(range(args.num_steps)):
            ...
            delta = (
                buf_att.rewards[t]
                + args.gamma * next_value * next_nonterm
                - buf_att.values[t]
            )
            adv_att[t] = last_gae = (
                delta
                + args.gamma * args.gae_lambda * next_nonterm * last_gae
            )

        ret_att = adv_att + buf_att.values

        # -------------------------------------------------
        # GAE / returns – defender
        # -------------------------------------------------
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, batch_size, minibatch):
                ...
                logits, values = attacker(
                    b_obs_att[mb],
                    action_mask=b_masks_att[mb],
                )
        ...
        torch.save(
            ...
            os.path.join(args.save_dir, "attacker_multi_ppo_latest.pt"),
        )
        ...
        torch.save(
            ...
            os.path.join(args.save_dir, "defender_multi_ppo_latest.pt"),
        )
